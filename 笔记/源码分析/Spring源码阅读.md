...
">
    ...
    <bean id="messageService" class="com.wyjie.impl.MessageServiceImpl"></bean>
...
public class App 
{
    public static void main( String[] args )
    {
        ...
        System.out.println("context 启动成功了");
...
在这个方法中会**创建bean容器**，**加载并注册bean**，但是这些**bean都没有初始化**
...
/**
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
        //关闭旧的bean，创建新的BeanFactory，加载Bean定义，注册Bean等
		refreshBeanFactory();
        //返回刚刚创建的BeanFactory
...
/**
	...
	protected final void refreshBeanFactory() throws BeansException {
        //如果存在BeanFactory，销毁所有的Bean，关闭BeanFactory。
        //这里指的是当前ApplicationContext是否有BeanFactory
		if (hasBeanFactory()) {
			destroyBeans();
			closeBeanFactory();
		}
		try {
			...
			//设置BeanFactory的两个配置属性：1.是否允许Bean覆盖2.是否允许循环引用
...
**由此我们可以了解到，ApplicationContext虽然继承自BeanFactory，但是它不应该被理解为BeanFactory的实现类，而是说ApplicationContext内部有一个实例化的BeanFactory（DefaultListableBeanFactory）。以后所有的BeanFactory的相关操作都交给这个实例来处理**

###### 为什么实例化`DefaultListableBeanFactory`

![image-20200703115640047](F:\java书和视频\笔记\源码分析\imgs\1.png)

`DefaultListableBeanFactory`是属于最底层的类，实现了是有的类

###### BeanDefinition接口定义

我们常说的Bean就是指BeanDefinition，我们自己定义的各个Bean会转换成一个个BeanDefinition存在于Spring的BeanFactory中

**BeanDefinition保存了Bean信息，比如这个Bean指向的是哪个类，是否是单例的，是否懒加载，这个Bean依赖了哪些Bean等**
...
该方法根据配置，加载各个Bean，然后放到BeanFactory中
...
/**
	...
	protected void loadBeanDefinitions(DefaultListableBeanFactory beanFactory) throws BeansException, IOException {
        ...
        //真正加载BeanDefinition到BeanFactory中的方法
		loadBeanDefinitions(beanDefinitionReader);
	}
```

###### 真正执行加载的`loadBeanDefinitions`方法
...
protected void loadBeanDefinitions(XmlBeanDefinitionReader reader) throws BeansException, IOException {
   ...
   if (configLocations != null) {
      // 2 通过解析路径转换到上面那个方法，获得Resource对象
...
`parseDefaultElement`解析的节点是`<import/>,<alias/>,<bean/>,<beans/>`这四个

这四个标签之所以是`default`，是因为它们处于这个namespace下定义的：

```xml
http://www.springframework.org/schema/beans 
```

其他标签属于扩展标签，就到`parseCustomElement`方法中解析

#### 准备Bean容器：`prepareBeanFactory`

Spring把我们在xml配置的bean都注册以后，会“手动”注册一些特殊的bean
...
该步骤的重点是`finishBeanFactoryInitialization(beanFactory)`，它会负责初始化所有的singleton beans

在这个方法之前，BeanFactory已经创建完成，并且所有的实现了BeanFactoryPostProcessor接口的Bean都已经初始化并且其中的postProcessBeanFactory(factory)方法已经得到了回调执行了。而且Spring已经“手动”注册了一些特殊的Bean，如`evironment`、`systemProperties`
